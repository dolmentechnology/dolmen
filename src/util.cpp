...
const char * const BITCOIN_CONF_FILENAME = "dolmen.conf";
const char * const BITCOIN_PID_FILENAME = "dolmend.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "dolmen" is a composite category enabling all Dolmen-related debug output
            if(ptrCategory->count(string("dolmen"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DolmenCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DolmenCore
    // Mac: ~/Library/Application Support/DolmenCore
    // Unix: ~/.dolmencore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DolmenCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DolmenCore";
#else
    // Unix
    return pathRet / ".dolmencore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty dolmen.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override dolmen.conf
